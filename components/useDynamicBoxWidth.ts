...
export function useDynamicBoxWidth() {
  ...
  const boxWidth = useMemo(() => {
    ...
    const scrollContainerPadding = 32; // p-4 = 16px each side
    const scrollbarWidth = 20; // Non-overlay scrollbars on Windows/Linux
    const availableWidth = maxContainerWidth - containerPadding - scrollContainerPadding - scrollbarWidth;

    // Convert to character count. Use conservative estimate (9.5px) to account for
    // wider system monospace fonts like Courier New which can be ~9.5px at text-sm (14px)
    const approxCharWidth = 9.5;
